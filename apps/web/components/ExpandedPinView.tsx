            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-7 pb-[60px] custom-vertical-scrollbar">
                    ...
                    <div className="flex justify-between items-start mb-5">
                        <div>
                            <span className="font-cubao-wide text-[12px] tracking-[0.1em] uppercase" style={{ color }}>
                                {pin?.pinTags?.map((pt) => pt.tag.title).join(", ")}
                            </span>
                            <h2 className="font-chakra text-primary text-[26px] font-extrabold m-0 mt-1">{pin?.title}</h2>
                        </div>
                    ...
                    {pin?.images && pin.images.length > 0 ? (
                        <div className="grid grid-rows-[repeat(2,90px)] grid-flow-col gap-3 mb-6 overflow-x-auto overscroll-x-contain pb-2 snap-x snap-mandatory custom-scrollbar no-scrollbar">
                            {pin.images.map((img) => (
                                <div key={img.id} className="bg-panel-hover border border-border-color rounded-2xl flex items-center justify-center snap-start relative overflow-hidden row-span-2 w-[192px]">
                    ...
                    <div className="flex flex-col gap-6">
                        <p className="font-nunito text-[15px] text-primary leading-relaxed m-0">{pin?.description}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 bg-panel-hover border border-border-color rounded-2xl p-5">
                            ...
                            <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container">
                                <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">PIN ID</span>
                            ...
                            {/* OWNER */}
                            <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container">
                                <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">CREATED BY</span>
                                <span className="font-nunito text-[14px] font-bold text-primary whitespace-nowrap w-max bg-[length:200%_200%] animate-scroll-text overflow-hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{pin?.owner}</span>
                            </div>
                            {/* COORDINATES */}
                            <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container col-span-2">
                                <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">COORDINATES (LAT, LNG)</span>
                                <span className="font-nunito text-[14px] font-bold text-primary whitespace-nowrap w-max bg-[length:200%_200%] animate-scroll-text overflow-hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden font-mono">
                            ...
                            {/* STATUS */}
                            <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container">
                                <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">STATUS</span>
                                <span
                            ...
                            {/* TIMESTAMPS */}
                            <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container">
                                <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">CREATED AT</span>
                            ...
                            {pin.modifications.filter((m) => m.status === "APPLIED").length > 0 && (
                                <div className="flex flex-col gap-1 min-w-0 overflow-hidden @container col-span-2">
                                    <span className="font-chakra text-[10px] font-extrabold text-secondary tracking-[0.1em]">LAST UPDATED</span>
                                    <span className="font-nunito text-[14px] font-bold text-primary whitespace-nowrap w-max bg-[length:200%_200%] animate-scroll-text overflow-hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                        ...
                        {isDeleting && (
                            <div className="bg-panel-hover border border-status-danger rounded-xl p-4 flex flex-col gap-3">
                                <p className="m-0 text-primary font-nunito text-[14px]">Are you sure you want to permanently delete this pin?</p>
                        ...
                        <div className="mt-2 border-t border-border-color pt-6 pb-[100px]">
                            <h3 className="font-chakra text-[12px] font-black tracking-[0.15em] text-secondary mb-5">FORUM</h3>
                            {!isReplying ? (
                                sessionData && (
                                    <button
                                        type="button"
                                ...
                                >
                                    ...
                                    />
                                    <button type="submit" className="tactical-button-primary px-4 py-2.5 text-[11px] rounded-md">
                                        POST
                                    </button>
                                    <button
                                        type="button"
